...
namespace Matrices {
    ...
    const double Matrix::EPSILON = 0.001; // Define epsilon tolerance for equality check

    Matrix::Matrix(int _rows, int _cols) {
        rows = _rows;
        cols = _cols;
        a = std::vector<std::vector<double>>(rows, std::vector<double>(cols, 0));
    }
    ...
    bool operator==(const Matrix& a, const Matrix& b) {
        ...
        for (int r = 0; r < a.getRows(); r++) {
            for (int c = 0; c < a.getCols(); c++) {
                if (abs(a(r, c) - b(r, c)) >= Matrix::EPSILON) {
                    return false;
                }
            }
        }
